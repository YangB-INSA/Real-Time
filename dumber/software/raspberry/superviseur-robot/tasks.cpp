...
#define PRIORITY_TCLOSECAMERA 20
#define PRIORITY_TWDRST 25
#define PRIORITY_TBATTERY 20 
#define PRIORITY_TRESTART 23
...
void Tasks::Init() {
    ...
    if (err = rt_mutex_create(&mutex_comRobot_err, NULL)) {
        cerr << "Error mutex create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&th_wdReset, "th_wdReset", 0, PRIORITY_TWDRST, 0)) {
        cerr << "Error task create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    if (err = rt_task_create(&th_restartserver, "th_restartserver", 0, PRIORITY_TRESTART, 0)) {
        cerr << "Error task create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
...
void Tasks::Run() {
    ...
    if (err = rt_task_start(&th_wdReset, (void(*)(void*)) & Tasks::WatchDogResetTask, this)) {
        cerr << "Error task start: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    if (err = rt_task_start(&th_checkrobot, (void(*)(void*)) & Tasks::CheckComRobot, this)) {
        cerr << "Error task start: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
...
void Tasks::RestartServerTask(void *arg) {
    ...
    /**************************************************************************************/
    /* The task restartserver starts here                                                        */
    /**************************************************************************************/
    
    while(1) {
        rt_sem_p(&sem_restartserver, TM_INFINITE);
        
        cout << "--- Reinitialisation du superviseur ---" << endl << flush;
        
        cout << "--- Fermeture comRobot ---" << endl << flush;
        //on arrete la communication avec le robot (+ on le stop)
        rt_sem_v(&sem_closeComRobot);
        
        cout << "--- Fermeture comCamera ---" << endl << flush;
        //on arrete la caméra
        rt_sem_v(&sem_closeCamera);
        
        cout << "--- Fermeture comMonitor ---" << endl << flush;
        //on ferme le serveur du monitor
        rt_mutex_acquire(&mutex_monitor, TM_INFINITE);
        monitor.Close();
        cout << "Close server on port(5544)" << endl << flush;
        rt_mutex_release(&mutex_monitor);
        
        cout << "--- Reouverture comMonitor ---" << endl << flush;
        //on réouvre le server pour réaccepter la com avec le monitor
        rt_sem_v(&sem_startserver);            
    }
}
...
void Tasks::ReceiveFromMonTask(void *arg) {
    ...
    while (1) {
        ...
        if (msgRcv->CompareID(MESSAGE_MONITOR_LOST) && cont_err == 0) {  
            cont_err++;       
            cout << "-----Perte de communication avec le moniteur-----"<< endl << flush;         
...
void Tasks::CloseComRobot(void *arg) {
    ...
    while (1) {
        ...
        //cout << "-----Fermeture de la communication avec le robot-----"<<endl <<flush;
        rt_mutex_acquire(&mutex_robot, TM_INFINITE);
        cout << "Close serial com ("<<flush;
        status = robot.Close();
        rt_mutex_release(&mutex_robot);
        cout << status;
        cout << ")" << endl << flush;

        //cout << "------Réinitialisation des variables partagées.------ \n" << endl <<flush;
...
void Tasks::CloseCamera(void *arg) {
    ...
    while (1) {
        ...
        //cout << "-----Deconnexion de la caméra-----"<<endl <<flush;
...
*/
void Tasks::WatchDogResetTask(void *args) {
    int wd, rs;
    Message* answer;
...
void Tasks::CheckComRobot(void *arg) {
    ...
    while (1) {
        rt_task_wait_period(NULL);
        //cout << "Periodic check ComRobot " << comRobot_err << endl;
        ...
        if (rs == 1 && wd == 1) {
            ...
            if (error_cnt >=3) {
               ...
               error_cnt++;  
               cout << "ComRobot ERROR : " << error_cnt << endl << flush;
            }      
            else {
               error_cnt=0;          
            }
